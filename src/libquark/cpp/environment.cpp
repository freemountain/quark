...
QString Environment::getScriptPath() {
  ...
  bool splitArgs =
      this->appArguments.size() > 3 && this->appArguments.at(2) == "--";

  if (appArguments.size() > 1) return appArguments.at(1);
...
#ifdef _WIN32
  nodePath = QFileInfo(QCoreApplication::applicationFilePath()).absolutePath() +
             "/resources/node_path";
#elif __linux__
  nodePath = QFileInfo(QCoreApplication::applicationFilePath()).absolutePath() +
             "/resources/node_path";
// nodePath = QDir( QCoreApplication::applicationFilePath() +
// "/../../resources/" ).absoluteFilePath("node_path");

#elif __APPLE__
  nodePath = QDir(QCoreApplication::applicationFilePath() + "/../../Resources/")
                 .absoluteFilePath("node_path");
#endif
...
QuarkProcess* Environment::startProcess(QString path) {
  Either<QMap<QString, QString>, QJsonParseError> mayJson =
      this->loadJson(path);

  if (mayJson.is2nd()) {
    this->printLine("Could not parse: " + path + "error: " +
                    mayJson.as2nd().errorString());
    return nullptr;
  }
  ...
  QStringList arguments =
      QStringList() << main << "--dataPath"
                    << this->getDataPath().filePath(name) << "--configPath"
                    << this->getConfigPath() << "--shellPath"
                    << QCoreApplication::applicationFilePath();

  QuarkProcess* proc = new QuarkProcess(this->getProcEnv(), this, this);

  if (json.contains("initialQml"))
    proc->handleLoadQml(json.value("initialQml"));

  connect(proc, &QuarkProcess::startProcess,
          [this](const QString& path) { this->startProcess(path); });
...
Either<QMap<QString, QString>, QJsonParseError> Environment::loadJson(
    QString path) {
  QString data;
  QString main;
  QString initialQml;
  ...
  if (initialQml != "") {
    result.insert("initialQml", QDir::toNativeSeparators(
                                    baseDir.absolutePath() + "/" + initialQml));
  }

  result.insert("main",
                QDir::toNativeSeparators(baseDir.absolutePath() + "/" + main));
